...
public class TieredTooltip {
    ...
    public static List<ClientTooltipComponent> buildComponents(List<Component> text, Optional<TooltipComponent> data, ItemStack stack) {
        ...
        List<ClientTooltipComponent> components = new ArrayList<>(text.size() + 1);

        for (Component line : text) {
            if (ReforgeMaterialTooltip.isSlotsMarker(line)) {
                if (shift) {

                    for (Component desc : ReforgeMaterialTooltip.slotsMarkerToDescriptions(line)) {
                        components.add(ClientTooltipComponent.create(desc.getVisualOrderText()));
                    }
                } else {
        ...
        data.ifPresent(d -> components.add(text.isEmpty() ? 0 : 1, ClientTooltipComponent.create(d)));
        return components;
    }
    ...
    public static void renderTieredTooltipFromComponents(GuiGraphics context, Font textRenderer, List<ClientTooltipComponent> components, int x, int y, ClientTooltipPositioner positioner,
        ...
        context.blit(borderTemplate.getIdentifier(), n - 6, o - 6, secondHalf * 64, border * 16, 8, 8, 128, 128);

        context.blit(borderTemplate.getIdentifier(), n + l - 2, o - 6, 56 + secondHalf * 64, border * 16, 8, 8, 128, 128);

        context.blit(borderTemplate.getIdentifier(), n - 6, o + m - 2, secondHalf * 64, 8 + border * 16, 8, 8, 128, 128);

        context.blit(borderTemplate.getIdentifier(), n + l - 2, o + m - 2, 56 + secondHalf * 64, 8 + border * 16, 8, 8, 128, 128);

        context.blit(borderTemplate.getIdentifier(), (n - 6 + n + l + 6) / 2 - 24, o - 9, 8 + secondHalf * 64, border * 16, 48, 8, 128, 128);
