...
public class NoSlotConfirmScreen extends Screen {
    ...
    private List<OrderedText> bodyLines = List.of();
    private int panelX, panelY, panelW, panelH;
    private float time;

    public NoSlotConfirmScreen(Screen parent, Runnable onConfirm, Runnable onConfirmDontAsk) {
        super(Text.translatable("screen.tiered.reforge.confirm.no_slot.title"));
        this.parent = parent;
    ...
    @Override
    protected void init() {
        bodyLines = this.textRenderer.wrapLines(
                Text.translatable("screen.tiered.reforge.confirm.no_slot.body"), PANEL_W - 32);

        int header = 48;
        int bodyH  = bodyLines.size() * (this.textRenderer.fontHeight + 2) + 10;
        ...
        buttons[0] = new Btn(bx, by, Text.translatable("screen.tiered.reforge.confirm.no_slot.yes"),
                BtnStyle.PRIMARY, () -> { onConfirm.run(); back(); });
        buttons[1] = new Btn(bx, by + BTN_H + BTN_GAP, Text.translatable("screen.tiered.reforge.confirm.no_slot.yes_dont_ask"),
                BtnStyle.SECONDARY, () -> { onConfirmDontAsk.run(); back(); });
        buttons[2] = new Btn(bx, by + 2 * (BTN_H + BTN_GAP), Text.translatable("gui.cancel"),
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        float pulse = (MathHelper.sin(time * 0.15f) + 1f) * 0.5f;
        int glow = blend(BORDER, BORDER_GLOW, pulse);

        context.drawBorder(panelX - 2, panelY - 2, panelW + 4, panelH + 4, withAlpha(glow, 0x44));

        context.fillGradient(panelX, panelY, panelX + panelW, panelY + panelH, PANEL_FILL_TOP, PANEL_FILL);
        context.drawBorder(panelX, panelY, panelW, panelH, glow);
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, this.title, cx, titleY, ACCENT);
        ...
        for (OrderedText line : bodyLines) {
            ...
            context.drawTextWithShadow(this.textRenderer, line, cx - lw / 2, by, BODY);
            by += this.textRenderer.fontHeight + 2;
        }
    ...
    private void drawWarningCrest(DrawContext context, int cx, int cy, float pulse) {
        int r = 9;
    ...
    private final class Btn {
        ...
        Btn(int x, int y, Text label, BtnStyle style, Runnable action) {
        ...
        void render(DrawContext context, int mouseX, int mouseY) {
            ...
            context.drawBorder(x, y, BTN_W, BTN_H, hovered ? style.borderHover : style.border);

            if (hovered) context.fill(x + 1, y + 1, x + BTN_W - 1, y + 2, withAlpha(style.borderHover, 0x55));

            int tw = NoSlotConfirmScreen.this.textRenderer.getWidth(label);
            int tx = x + (BTN_W - tw) / 2;
            int ty = y + (BTN_H - NoSlotConfirmScreen.this.textRenderer.fontHeight) / 2 + 1;
            context.drawTextWithShadow(NoSlotConfirmScreen.this.textRenderer,
                    label.copy().styled(s -> s.withColor(hovered ? 0xFFFFFFFF : style.text)), tx, ty, style.text);
        }
