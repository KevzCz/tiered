...
public record ImprintComponent(List<Slot> slots) {
    ...
    public static final PacketCodec<RegistryByteBuf, Entry> ENTRY_PACKET = PacketCodec.tuple(
            ...
            PacketCodecs.INTEGER, Entry::count,
            PacketCodecs.FLOAT, Entry::value,
            PacketCodecs.map(HashMap::new, PacketCodecs.STRING, PacketCodecs.FLOAT), Entry::extraValues,
            Entry::new
    );

    public static final PacketCodec<RegistryByteBuf, Slot> SLOT_PACKET = PacketCodec.tuple(
            PacketCodecs.STRING, Slot::sourceId,
            ENTRY_PACKET.collect(PacketCodecs.toList()), Slot::entries,
            Slot::new
    );

    public static final PacketCodec<RegistryByteBuf, ImprintComponent> PACKET_CODEC = PacketCodec.tuple(
            SLOT_PACKET.collect(PacketCodecs.toList()), ImprintComponent::slots,
