...
public class DataEffect extends ReforgeEffect {
    ...
    public boolean canRun(ItemStack target) {
        ...
        return switch (def.getType()) {
            ...
            case "repair" -> target.isDamageable() && target.isDamaged();
    ...
    public void preReforge(PlayerEntity player, ItemStack stack, EffectContext context, RollBias rollBias) {
        ...
        switch (def.getType()) {
            case "stabilize" -> {
                Identifier current = ModifierUtils.getAttributeId(stack);
                if (current != null) {
    ...
    @Override
    public boolean onReforge(PlayerEntity player, ItemStack stack, EffectContext context) {
    ...
    private String additionId(EffectContext context) {
        ...
        if (addition != null && !addition.isEmpty()) {
            Identifier itemId = Registries.ITEM.getId(addition.getItem());
    ...
    private boolean doRepair(ItemStack stack) {
        ...
        stack.setDamage(Math.max(0, damage - repaired));
        return true;
    }
