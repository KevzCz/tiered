...
public class ReforgeMaterial {
    ...
    public static class ImprintPoolChoice {
        ...
        public ImprintPoolChoice(List<Candidate> candidates) {
            this.candidates = candidates;
        }
        ...
        public ImprintPool pick(Random rng) {
            List<Candidate> pool = getCandidates();
            if (pool.isEmpty()) return null;
            int total = 0;
            for (Candidate c : pool) total += c.getWeight();
    ...
    public static class EffectPoolChoice {
        ...
        public EffectPoolChoice(List<Candidate> candidates) {
            this.candidates = candidates;
        }
        ...
        public EffectPool pick(Random rng) {
            return weightedPickChoice(getCandidates(), rng, Candidate::getWeight, Candidate::getPool);
        }
    ...
    public static class BehaviorPoolChoice {
        ...
        public BehaviorPoolChoice(List<Candidate> candidates) {
            this.candidates = candidates;
        }
    ...
    public static class BehaviorPool {
        ...
        public BehaviorPool(Integer rollMin, Integer rollMax, Integer nothingWeight, List<Candidate> candidates) {
            this.rollMin = rollMin;
            this.rollMax = rollMax;
            this.nothingWeight = nothingWeight;
            this.candidates = candidates;
        ...
        public int getRollMin() { return rollMin == null ? 1 : Math.max(0, rollMin); }
        public int getRollMax() { return rollMax == null ? getRollMin() : Math.max(getRollMin(), rollMax); }
        public List<Candidate> getCandidates() { return candidates == null ? List.of() : candidates; }

        @Nullable
        Candidate roll(Random rng) {
            List<Candidate> pool = getCandidates();
            if (pool.isEmpty()) return null;
            int min = getRollMin();
            if (min == 0) {
                int candidateTotal = 0;
                for (Candidate c : pool) candidateTotal += c.getWeight();
                int nw = nothingWeight != null ? Math.max(0, nothingWeight) : candidateTotal;
                int total = candidateTotal + nw;
        ...
        private static Candidate weightedPick(List<Candidate> candidates, Random rng) {
            int total = 0;
            for (Candidate c : candidates) total += c.getWeight();
            if (total <= 0) return candidates.get(rng.nextInt(candidates.size()));
            int roll = rng.nextInt(total);
