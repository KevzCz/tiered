...
public class ImprintPlatesComponent implements TooltipComponent {
    ...
    private static final Identifier RUNE_FRAME =
            Identifier.of("kevs", "textures/item/rune_frame.png");
    ...
    private int plateWidth(TextRenderer tr, ImprintPlatesData.Plate plate) {
        return PAD_X + tr.getWidth(plate.label()) + PAD_X;
    }

    private List<List<ImprintPlatesData.Plate>> wrapPlates(TextRenderer tr, ImprintPlatesData.SlotView slot) {
    ...
    private int slotHeight(TextRenderer tr, ImprintPlatesData.SlotView slot) {
    ...
    private int slotRowWidth(TextRenderer tr, ImprintPlatesData.SlotView slot) {
        int max = 0;
        ...
        for (String d : detailLines(slot)) max = Math.max(max, tr.getWidth(d));
        return ICON + ICON_GAP + max;
    }

    private int headerRowWidth(TextRenderer tr) {
        int w = header == null ? 0 : tr.getWidth(header);
    ...
    private int computeHeight(TextRenderer tr) {
    ...
    @Override
    public void drawText(TextRenderer tr, int x, int y, Matrix4f matrix, VertexConsumerProvider.Immediate vc) {
        int headerH = headerRowHeight();
        if (header != null) {
            tr.draw(header, x, y + TOP_GAP + (headerH - 8) / 2, 0xFFAAAAAA, false, matrix, vc,
        ...
        for (ImprintPlatesData.SlotView slot : slots) {
            ...
            for (List<ImprintPlatesData.Plate> row : rows) {
                ...
                for (ImprintPlatesData.Plate plate : row) {
                    ...
                    tr.draw(plate.label(), px + PAD_X, textY, textColor, true, matrix, vc,
                            TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
            ...
            for (String d : details) {
                tr.draw(d, x + ICON + ICON_GAP, detailY, 0xFF888888, false, matrix, vc,
                        TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
    ...
    @Override
    public void drawItems(TextRenderer tr, int x, int y, DrawContext context) {
        RenderSystem.enableBlend();

        int headerH = headerRowHeight();
        if (emptySlots > 0) {
            int hx = x + (header == null ? 0 : tr.getWidth(header) + HEADER_GAP);
            int hy = y + TOP_GAP + (headerH - ICON) / 2;
            for (int i = 0; i < emptySlots; i++) {
                context.drawTexture(RUNE_FRAME, hx, hy, ICON, ICON, 0, 0, 32, 32, 32, 32);
                hx += ICON + H_GAP;
            }
        }
        ...
        for (ImprintPlatesData.SlotView slot : slots) {
            ...
            if (slot.rune() != null && !slot.rune().isEmpty()) {
                context.drawItem(slot.rune(), x, iconY);
            } else {
                context.drawTexture(RUNE_FRAME, x, iconY, ICON, ICON, 0, 0, 32, 32, 32, 32);
            }
    ...
    private static String plateDetail(ImprintPlatesData.Plate plate) {
        ...
        String works = Text.translatable("tiered.imprint.works_with").getString()
                + ": " + String.join(", ", tags);
        String label = plate.label() == null ? "" : plate.label().getString();
        return label.isEmpty() ? works : label + " — " + works;
    }

    private static int drawRequirementIcon(DrawContext context, ImprintPlatesData.Plate plate, int px, int plateRowY) {
    ...
    public static int drawStandalonePlate(DrawContext context, TextRenderer tr, int x, int y, Text label, int fill) {
        int w = PAD_X + tr.getWidth(label) + PAD_X;
        RenderSystem.enableBlend();
        drawPlate(context, x, y, w, PLATE_H, fill, 1.0f, false);
        RenderSystem.disableBlend();
        int textY = y + (PLATE_H - 8) / 2;
        context.drawText(tr, label, x + PAD_X, textY, 0xFFFFFFFF, true);
        return w;
    }

    private static void drawPlate(DrawContext context, int x, int y, int w, int h, int fill, float cooldownFill,
            boolean active) {
