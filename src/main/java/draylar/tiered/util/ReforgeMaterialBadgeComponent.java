...
public class ReforgeMaterialBadgeComponent implements TooltipComponent {
    ...
    private int badgeContentWidth(TextRenderer tr, ReforgeMaterialBadgeData.Badge badge) {
        int glyphW = badge.glyph().isEmpty() ? 0 : tr.getWidth(badge.glyph()) + GLYPH_GAP;
        return PAD_X + glyphW + tr.getWidth(badge.label()) + PAD_X;
    }
    ...
    public int getWidth(TextRenderer textRenderer) {
        ...
        for (ReforgeMaterialBadgeData.Badge badge : badges) {
            ...
            if (hasDesc(badge)) {
                max = Math.max(max, PAD_X + textRenderer.getWidth(badge.description()));
            }
        }
        for (Text line : lore) {
    ...
    public void drawText(TextRenderer textRenderer, int x, int y, Matrix4f matrix, VertexConsumerProvider.Immediate vertexConsumers) {
        ...
        for (ReforgeMaterialBadgeData.Badge badge : badges) {
            ...
            if (!badge.glyph().isEmpty()) {
                textRenderer.draw(Text.literal(badge.glyph()), tx, textY, 0xFFFFFFFF, true, matrix, vertexConsumers,
                        TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
                tx += textRenderer.getWidth(badge.glyph()) + GLYPH_GAP;
            }
            textRenderer.draw(badge.label(), tx, textY, 0xFFFFFFFF, true, matrix, vertexConsumers,
                    TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
            by += BADGE_H + BADGE_GAP;
            if (hasDesc(badge)) {
                textRenderer.draw(badge.description(), x + PAD_X, by, 0xFFAAAAAA, false, matrix, vertexConsumers,
                        TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
                by += DESC_H;
            }
        }
        ...
        for (Text line : lore) {
            textRenderer.draw(line, x, ly, 0xFFAAAAAA, false, matrix, vertexConsumers,
                    TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
            ly += LINE_H;
        }
    }
    ...
    private static void drawPlate(DrawContext context, int x, int y, int w, int h, int fill) {
